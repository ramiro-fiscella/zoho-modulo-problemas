...
└── Problemas/
    ...
    └── 05-funcion-standalone/               ← función Deluge para el portal de Comerciales
```
...
### 5. Función standalone — portal de Comerciales

Carpeta: `05-funcion-standalone/`

Crea un ticket desde el portal propio del área Comercial (externo al CRM). Hace lo
mismo que el widget (mismo payload) pero como función Deluge, y crea el registro con
trigger de workflow para que se dispare `problemas_post_create`.

**Subir a Zoho:**
Setup → Functions → nueva función standalone → nombre `problemas_create_standalone`
→ pegar el `.deluge`. Para que el portal la llame: pestaña REST API → generar URL
(⚠️ la key va en el backend del portal, no en el navegador).
...
- ✅ Función standalone (`problemas_create_standalone`) para crear tickets desde el
  portal de Comerciales, con disparo del workflow post-create.
